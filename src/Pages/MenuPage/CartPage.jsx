...
const CartProduct = () => {
  ...
  return (
    <section className="grid grid-cols-1 md:grid-cols-2">
      ...
      <div className="">
        <div className="bg-[#eaeaea] min-h-screen w-full flex items-center justify-center overflow-y-auto ">{count >= 1 ? <div>{Array.isArray(cartItems) && cartItems.map((cart, index) => <Cart key={index} product={cart} name={cart.name} _id={cart} />)}</div> : <Order />}</div>
        <div className="">
          <Footer />
        </div>
      </div>
